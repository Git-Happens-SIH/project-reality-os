...
(async()=>{
 ...
 const base=process.env.DEMO_URL||'http://127.0.0.1:8000/';const go=page=>p.goto(base+'project-reality-os-'+page+'.html',{waitUntil:'domcontentloaded'});
 ...
 const dashboard=await context.newPage();await dashboard.goto(base+'project-reality-os-dashboard.html');assert.equal(await dashboard.locator('.confirmed .stat-num').innerText(),'18');
